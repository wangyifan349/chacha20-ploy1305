...
This script is used to recursively encrypt or decrypt all files under a specified directory. After the program starts, it first runs a startup self-check 10 times.
Only after all self-check rounds pass does the user enter a password, then enter the target directory path, and finally choose the operation type: enter e for
encryption, or d for decryption. The script does not hardcode the key or password in the source code. Instead, it derives a 32-byte key from the password entered
at runtime, and uses that key for subsequent ChaCha20-Poly1305 encryption and authentication.

Core logic:

1. Startup self-check:
   Before processing any user file, the script repeatedly verifies the internal encryption and decryption path in memory. Each self-check round confirms that
   ChaCha20-Poly1305 encryption can process random plaintext, produce ciphertext and an authentication tag, decrypt valid ciphertext back to the original
   plaintext, reject tampered ciphertext, derive a 32-byte key with PBKDF2, and obtain random bytes from the operating system.

2. Key derivation:
   The user-entered password is not used directly as the ChaCha20-Poly1305 key. Instead, PBKDF2-HMAC-SHA256 is used to derive a fixed-length 32-byte key.
   This avoids hardcoding a key in the source code and provides better resistance against brute-force attacks than a simple sha256(password). To keep the
   original file format unchanged, the current version uses a fixed salt. If the file format can be changed in the future, this can be further improved by
   using an independent random salt for each file.

3. File encryption:
   When encrypting each individual file, the program generates a random 12-byte nonce, then uses ChaCha20 to encrypt the file plaintext into ciphertext.
   At the same time, Poly1305 authenticates the ciphertext and generates a 16-byte authentication tag. The final file format remains:
       nonce(12 bytes) || tag(16 bytes) || ciphertext

4. File decryption:
   During decryption, the program reads the first 12 bytes of the file as the nonce, the next 16 bytes as the tag, and the remaining bytes as the ciphertext.
   It then derives the key again from the same user-entered password and first verifies the Poly1305 authentication tag. If authentication fails, it means the
   password is wrong, the file has been tampered with, or the file content is corrupted, and the program refuses to decrypt it. Only after authentication
   succeeds does the program use ChaCha20 to recover the plaintext.

5. Directory processing:
   If the target is a directory, the script recursively traverses all files in the directory using os.walk, and calls encrypt_file or decrypt_file for each file.
   Each file gets its own nonce, so even when the same user password is used, the encrypted output of different files is different.

Notes:
- This script overwrites files in place. Encryption replaces the original file with ciphertext, and decryption replaces the encrypted file with plaintext.
  Back up important data before using it in practice.
- Encryption and decryption must use the same password. If the password is lost, the original files cannot be recovered.
- This script is mainly intended for learning and personal use. In production environments, prefer mature cryptographic libraries, such as the
  ChaCha20Poly1305 implementation from Python's cryptography library.
"""
...
def chacha20_block(key: bytes, counter: int, nonce: bytes) -> bytes:
    ...
    for round_index in range(10):                                    # 10 double rounds = 20 ChaCha20 rounds.
    ...
    return struct.pack("<16I", *output_words)                        # Pack sixteen 32-bit integers into a 64-byte keystream block.
...
def chacha20_crypt(key: bytes, nonce: bytes, counter: int, data: bytes) -> bytes:
    ...
    for block_index in range(block_num):
        ...
        block_end = min(block_start + 64, data_length)               # The final block may be shorter than 64 bytes.
        data_block = data[block_start:block_end]                     # Current block of input data.
        for byte_index in range(len(data_block)):
            result_bytes.append(data_block[byte_index] ^ keystream_block[byte_index])  # Encryption and decryption are both data XOR keystream.
    return bytes(result_bytes)

def poly1305_clamp_r_s(key: bytes):
    """Parse a 32-byte Poly1305 key: first 16 bytes are clamped r; last 16 bytes are s."""
    if len(key) != 32:
...
def poly1305_mac(key: bytes, msg: bytes) -> bytes:
    ...
    return accumulator.to_bytes(16, "little")                        # Output the 16-byte authentication tag.

def pad16(data: bytes) -> bytes:
    """Pad data with zero bytes until its length is a multiple of 16."""
    remainder = len(data) % 16                                       # Check whether the current length is already a multiple of 16.
    if remainder == 0:
        return data                                                  # If already aligned, add no extra padding.
    return data + (b"\x00" * (16 - remainder))                       # Zero-pad to the 16-byte boundary as required by the AEAD format.
...
def chacha20_poly1305_encrypt(key: bytes, nonce: bytes, plaintext: bytes) -> tuple[bytes, bytes]:
    ...
    mac_data = pad16(aad) + pad16(ciphertext) + u64_le(len(aad)) + u64_le(len(ciphertext))  # AEAD authentication input.
...
def chacha20_poly1305_decrypt(key: bytes, nonce: bytes, ciphertext: bytes, tag: bytes) -> bytes:
    ...
    return chacha20_crypt(key, nonce, 1, ciphertext)                 # Decrypt only after authentication succeeds.

def initialize_self_check():
    """
    Run the startup self-check 10 times before accepting user input.
    The self-check only tests internal cryptographic functions in memory and does not read, write, encrypt, or decrypt any user file.
    Each round generates random plaintext in memory. The plaintext size is randomly selected between 1 MB and 150 MB.
    """
    print("Running startup self-check...")
    min_plaintext_size = 1 * 1024 * 1024                              # Minimum random plaintext size: 1 MB.
    max_plaintext_size = 150 * 1024 * 1024                            # Maximum random plaintext size: 150 MB.
    for check_round in range(1, 11):
        print(f"\nStartup self-check round {check_round}/10 started.")
        test_key = os.urandom(32)                                    # Use a fresh random 32-byte test key for each self-check round.
        print(f"Startup self-check round {check_round}/10: test key generation passed.")
        test_nonce = os.urandom(12)                                  # Use a fresh random 12-byte test nonce for each self-check round.
        print(f"Startup self-check round {check_round}/10: test nonce generation passed.")
        random_size_range = max_plaintext_size - min_plaintext_size + 1
        test_plaintext_size = min_plaintext_size + int.from_bytes(os.urandom(4), "little") % random_size_range  # Random size between 1 MB and 150 MB.
        print(f"Startup self-check round {check_round}/10: random plaintext size selected: {test_plaintext_size / (1024 * 1024):.2f} MB.")
        test_plaintext = os.urandom(test_plaintext_size)             # Generate random plaintext directly in memory.
        print(f"Startup self-check round {check_round}/10: random plaintext generation passed.")
        test_ciphertext, test_tag = chacha20_poly1305_encrypt(test_key, test_nonce, test_plaintext)  # Verify encryption.
        print(f"Startup self-check round {check_round}/10: encryption passed.")
        recovered_plaintext = chacha20_poly1305_decrypt(test_key, test_nonce, test_ciphertext, test_tag)  # Verify decryption.
        print(f"Startup self-check round {check_round}/10: decryption passed.")
        if recovered_plaintext != test_plaintext:
            raise RuntimeError(f"Startup self-check round {check_round} failed: plaintext mismatch.")
        print(f"Startup self-check round {check_round}/10: plaintext consistency check passed.")
        tampered_ciphertext = bytearray(test_ciphertext)             # Create a modified copy to test authentication failure.
        tampered_ciphertext[0] ^= 0x01                               # Flip one bit; Poly1305 verification must reject it.
        try:
            chacha20_poly1305_decrypt(test_key, test_nonce, bytes(tampered_ciphertext), test_tag)
            raise RuntimeError(f"Startup self-check round {check_round} failed: tampered ciphertext was accepted.")
        except ValueError:
            print(f"Startup self-check round {check_round}/10: tamper detection check passed.")
        test_derived_key = hashlib.pbkdf2_hmac("sha256", b"self-check-password", b"chacha20-poly1305-password-key", 300000, dklen=32)
        if len(test_derived_key) != 32:
            raise RuntimeError(f"Startup self-check round {check_round} failed: derived key length is invalid.")
        print(f"Startup self-check round {check_round}/10: PBKDF2 key derivation check passed.")
        test_random_nonce = os.urandom(12)                           # Verify that the system random source is available.
        if len(test_random_nonce) != 12:
            raise RuntimeError(f"Startup self-check round {check_round} failed: random nonce generation failed.")
        print(f"Startup self-check round {check_round}/10: system random nonce check passed.")
        print(f"Startup self-check round {check_round}/10 fully passed.")
    print("\nAll startup self-check rounds passed.")
...
def get_key_from_password() -> bytes:
    ...
    return hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt, 300000, dklen=32)  # Derive a 32-byte ChaCha20 key.

def encrypt_file(file_path: str, key: bytes):
    """Encrypt a single file in place. File storage format: nonce(12) || tag(16) || ciphertext."""
...
def decrypt_file(file_path: str, key: bytes):
    """Decrypt a single file in place. Input file format: nonce(12) || tag(16) || ciphertext."""
    if not os.path.isfile(file_path):
...
if __name__ == "__main__":
    try:
        initialize_self_check()                                      # Run 10 in-memory startup self-check rounds before handling user files.
