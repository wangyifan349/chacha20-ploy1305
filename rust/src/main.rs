...
// ----------------------------------------------------------
// Batch Encrypt GUI  ·  ChaCha20-Poly1305  ·  Multithread
// ----------------------------------------------------------
...
use eframe::{
    egui::{
        self, Color32, ProgressBar, RichText, Visuals, ViewportBuilder, TextStyle, FontId,
    },
    CreationContext,
};
use rand::random;
...
fn worker(encrypt: bool, dir: PathBuf, pwd: String, tx: Sender<BgMsg>) {
    ...
    tx.send(BgMsg::Total(files.len())).ok();
...
impl Gui {
    fn new(cc: &CreationContext<'_>) -> Self {
        ...
        // custom golden palette  (base / hover / active)
        let gold_base   = Color32::from_rgb(212, 175, 55);
        let gold_hover  = Color32::from_rgb(235, 195, 75);
        let gold_active = Color32::from_rgb(255, 215, 95);

        visuals.widgets.inactive.bg_fill        = gold_base;
        visuals.widgets.inactive.fg_stroke.color= Color32::BLACK;
        visuals.widgets.hovered.bg_fill         = gold_hover;
        visuals.widgets.hovered.fg_stroke.color = Color32::BLACK;
        visuals.widgets.active.bg_fill          = gold_active;
        visuals.widgets.active.fg_stroke.color  = Color32::BLACK;

        visuals.selection.bg_fill               = gold_base;
